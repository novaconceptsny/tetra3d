...
artworkCanvas.on("object:scaling", function (e) {
    ...
    let selectedArtwork = canvasState.assignedArtwork.filter(art => {
        return art.artworkId === scaledObject.id;
    })[0];

    let newScale = (defaultScales[scaledObject.id] / scaledObject.scaleX) * selectedArtwork.scale;
    selectedArtwork.setOverrideScale(newScale);
...
artworkCanvas.on("before:render", () => {
    applyCropData()
});
...
artworkCanvas.on("object:added", (rendered) => {
    if (!allCropDataApplied) {
        const croppedArtIds = getCroppedArtAssignments().map(art => {
